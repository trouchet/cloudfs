...
CloudFS providers implementam **Device Code Flow interativo** como método
primário. **Nenhum** suporta autenticação server-to-server (Client Credentials
ou Service Accounts).

**Impact**: Impossível usar CloudFS em cenários headless (CI/CD, cron jobs,
APIs).
...
**Branch Status**: `feat/sharepoint-client-credentials` - Ready for
implementation
...
**Root Cause**: JWT requires RSA-SHA256 signature using private key - needs
...
**Problem**: This is NOT Device Code Flow (RFC 8628) - it's Authorization Code
Flow with manual code input

**Better Solution**: Implement true Device Code Flow if Dropbox supports it, or
Client Credentials
...
**Branch**: `feat/sharepoint-client-credentials` **Status**: WIP - awaiting C++
implementation
...
1. ✅ Cron jobs (scheduled data sync)
1. ✅ API services (query SharePoint on demand)
1. ✅ Microservices (inter-service data access)
1. ✅ Batch processing (server-side data pipelines)
...
1. [ ] Create PR with Client Credentials for SharePoint
1. [ ] Document required environment variables

### Short Term (Next Session)

1. [ ] Implement OneDrive Client Credentials
1. [ ] Research Dropbox Device Code support
1. [ ] Create GDrive JWT signing feature branch

### Long Term (Roadmap)

1. [ ] Add Google Drive Service Account JWT
1. [ ] Improve Dropbox authentication UX
1. [ ] Add API Key support to VFS
1. [ ] Create enterprise authentication guide
...
- OpenSSL EVP:
  [OpenSSL Documentation](https://www.openssl.org/docs/man1.1.1/man3/EVP_DigestSign.html)
- Google Service Account JWT:
  [Google Docs](https://developers.google.com/identity/protocols/oauth2/service-account)
- Microsoft Identity Platform:
  [Microsoft Docs](https://docs.microsoft.com/en-us/azure/active-directory/develop/v2-oauth2-client-creds-grant-flow)
