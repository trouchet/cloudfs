...
This directory contains all build, test, and development automation scripts for the CloudFS project.
...
4. Installs pre-commit via pip
5. Installs commitlint via npm
6. Installs clang-format, shellcheck, cmake-format
7. Installs Git hooks
8. Creates secrets baseline
...
3. Configuration files (.commitlintrc.json, .pre-commit-config.yaml, etc.)
...
2. Removes old build directory
3. Creates new build directory
4. Runs CMake with Ninja generator
5. Runs Ninja build
6. Copies extension to project root

**Test process**:
1. Generates random token for VFS agent
2. Creates test data in /tmp
3. Starts cloudfs-agent
4. Tests: version, providers, secrets, ls(), stat(), du(), file reading
5. Cleans up agent and test data
...
4. Commit: `git commit -m "chore(scripts): update script_name"`
...
**All scripts follow shellcheck best practices and are tested on Linux/WSL and macOS.**
