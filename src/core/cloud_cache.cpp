...
bool CloudCache::GetItem(const std::string &scheme, const std::string &path, CloudItem &out) {
    ...
    if (Clock::now() > it->second.expires_at) { items_.erase(it); return false; }
    out = it->second.value; return true;
}

void CloudCache::PutItem(const std::string &scheme, const std::string &path, const CloudItem &item) {
    std::lock_guard<std::mutex> lk(mu_);
    if (items_.size() >= kMaxItems) EvictOldest(items_, kMaxItems * 3 / 4);
...
void CloudCache::InvalidatePrefix(const std::string &scheme, const std::string &prefix) {
    ...
    for (auto it = items_.begin(); it != items_.end(); ) {
        if (it->first.substr(0, pfx.size()) == pfx) it = items_.erase(it);
        else ++it;
    }
}

// ── Download URL cache ────────────────────────────────────────────────────────
bool CloudCache::GetDownloadUrl(const std::string &scheme, const std::string &item_id, std::string &out) {
    std::lock_guard<std::mutex> lk(mu_);
    auto it = urls_.find(MakeKey(scheme, item_id));
    if (it == urls_.end()) return false;
    if (Clock::now() > it->second.expires_at) { urls_.erase(it); return false; }
    out = it->second.value; return true;
}
...
void CloudCache::PutDownloadUrl(const std::string &scheme, const std::string &item_id,
    ...
    std::lock_guard<std::mutex> lk(mu_);
    if (urls_.size() >= kMaxItems * 2) EvictOldest(urls_, kMaxItems);
...
bool CloudCache::GetRootId(const std::string &scheme, const std::string &key, std::string &out) {
    ...
    if (Clock::now() > it->second.expires_at) { roots_.erase(it); return false; }
    out = it->second.value; return true;
}

void CloudCache::PutRootId(const std::string &scheme, const std::string &key, const std::string &id) {
    std::lock_guard<std::mutex> lk(mu_);
...
void CloudCache::ClearScheme(const std::string &scheme) {
    ...
    for (auto *m : {(void*)&items_, (void*)&urls_, (void*)&roots_}) (void)m; // suppress warning
    auto strip = [&pfx](auto &map) {
        for (auto it = map.begin(); it != map.end(); )
            if (it->first.substr(0, pfx.size()) == pfx) it = map.erase(it); else ++it;
    };
    strip(items_); strip(urls_); strip(roots_);
}

void CloudCache::ClearAll() {
    std::lock_guard<std::mutex> lk(mu_);
    items_.clear(); urls_.clear(); roots_.clear();
}
...
void CloudCache::EvictExpired(Map &map) {
    auto now = Clock::now();
    for (auto it = map.begin(); it != map.end(); )
        if (now > it->second.expires_at) it = map.erase(it); else ++it;
}

template<typename Map>
void CloudCache::EvictOldest(Map &map, size_t target) {
    EvictExpired(map);
