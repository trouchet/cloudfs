...
static void Throw(const std::string &msg) { if (!msg.empty()) throw IOException(msg); }
...
CloudFileHandle::CloudFileHandle(CloudFileSystem &fs, const std::string &path,
    ...
    : FileHandle(fs, path, flags), file_size(item_.size),
      root_id(root_id_), item(item_), open_flags(flags), backend(backend_) {}

CloudFileHandle::~CloudFileHandle() { Close(); }
...
void CloudFileSystem::SetAuth(const std::string &scheme,
...
std::vector<std::string> CloudFileSystem::RegisteredSchemes() const {
    ...
    for (auto &[k,_] : backends_) v.push_back(k);
...
bool CloudFileSystem::CanHandleFile(const string &path) {
    std::lock_guard<std::mutex> lk(mu_);
    for (auto &[_, b] : backends_) if (b->CanHandle(path)) return true;
    return false;
}

ICloudBackend &CloudFileSystem::BackendFor(const std::string &path) {
    auto *b = BackendForNoThrow(path);
    if (!b) Throw("cloudfs: no backend registered for URL: " + path);
    return *b;
}

ICloudBackend *CloudFileSystem::BackendForNoThrow(const std::string &path) {
    std::lock_guard<std::mutex> lk(mu_);
    for (auto &[_, b] : backends_) if (b->CanHandle(path)) return b.get();
    return nullptr;
}
...
bool CloudFileSystem::ResolvePath(ICloudBackend &backend, const std::string &url,
                                   CloudItem &out_item, std::string &out_root_id,
                                   std::string &err) {
    // 1. Check path cache
    ...
    if (!backend.ParseUrl(url, raw_root, path, err)) return false;

    // 3. Resolve root (cached separately)
    std::string root_id;
    if (!cache_.GetRootId(backend.Scheme(), raw_root, root_id)) {
        std::string tok;
        if (!GetToken(backend.Scheme(), tok, err)) return false;
        if (!backend.ResolveRoot(raw_root, tok, root_id, err)) return false;
    ...
    std::string tok;
    if (!GetToken(backend.Scheme(), tok, err)) return false;
    if (!backend.Stat(root_id, path, tok, out_item, err)) return false;
...
bool CloudFileSystem::EnsureDownloadUrl(ICloudBackend &backend, const std::string &root_id,
                                         ...
                                         std::string &err) {
    // Check cache first
    if (cache_.GetDownloadUrl(backend.Scheme(), item.id, item.download_url)) return true;
...
// ─── OpenFile ─────────────────────────────────────────────────────────────────
unique_ptr<FileHandle> CloudFileSystem::OpenFile(const string &path,
                                                  FileOpenFlags flags,
    ...
    if (flags.OpenForWriting()) {
        ...
        placeholder.name = file_name; placeholder.size = 0;
    ...
    // Read mode
    CloudItem item; std::string root_id;
...
void CloudFileSystem::Read(FileHandle &handle, void *buf, int64_t nr, idx_t loc) {
    ...
    auto try_read = [&]() -> int64_t {
        ...
        return h.backend.ReadRange(h.item, h.root_id, tok,
                                    (int64_t)loc, nr, static_cast<char*>(buf), err);
    };
...
void CloudFileSystem::FlushWrite(CloudFileHandle &h) {
    if (!h.upload_active || h.write_buffer.empty()) return;
    std::string err, tok;
    ...
    for (int64_t offset = 0; offset < total; offset += chunk) {
        ...
        Throw(!h.backend.UploadChunk(h.upload_session,
                                      h.write_buffer.data() + offset,
                                      offset, sz, last, tok, err)
              ? "cloudfs upload: " + err : "");
    }
...
// ─── Metadata ─────────────────────────────────────────────────────────────────
int64_t     CloudFileSystem::GetFileSize(FileHandle &h) { return dynamic_cast<CloudFileHandle&>(h).file_size; }
timestamp_t CloudFileSystem::GetLastModifiedTime(FileHandle &h) { return Timestamp::GetCurrentTimestamp(); }
FileType    CloudFileSystem::GetFileType(FileHandle &h) {
    return dynamic_cast<CloudFileHandle&>(h).item.is_folder ? FileType::FILE_TYPE_DIR : FileType::FILE_TYPE_REGULAR;
}
void CloudFileSystem::Truncate(FileHandle &h, int64_t sz) { dynamic_cast<CloudFileHandle&>(h).declared_size = sz; }
bool CloudFileSystem::Trim(FileHandle &, idx_t, idx_t) { return false; }
...
bool CloudFileSystem::FileExists(const string &path, optional_ptr<FileOpener>) {
    auto *b = BackendForNoThrow(path);
    if (!b) return false;
    CloudItem item; std::string root_id, err;
    if (!ResolvePath(*b, path, item, root_id, err)) return false;
...
bool CloudFileSystem::ListFiles(const string &dir,
                                 const std::function<void(const string&, bool)> &cb,
                                 FileOpener *) {
    auto *b = BackendForNoThrow(dir);
    if (!b) return false;
    CloudItem folder; std::string root_id, err;
    if (!ResolvePath(*b, dir, folder, root_id, err) || !folder.is_folder) return false;
    std::string tok;
    if (!GetToken(b->Scheme(), tok, err)) return false;
    std::string cursor;
    do {
        if (!b->ListFolder(root_id, folder.id, tok,
                            [&](const CloudItem &c){ cb(c.name, c.is_folder); },
...
// ─── Glob ─────────────────────────────────────────────────────────────────────
void CloudFileSystem::GlobRecursive(ICloudBackend &backend,
    ...
    do {
        ...
        bool ok = backend.ListFolder(root_id, folder_id, tok,
        ...
        for (auto &child : batch) {
            ...
            if (child.is_folder) {
                if (recursive)
                    GlobRecursive(backend, root_id, child.id, child_url,
                                   file_pattern, recursive, results, tok);
            } else {
                if (match == 0)
...
vector<OpenFileInfo> CloudFileSystem::Glob(const string &path, FileOpener *) {
    ...
    if (!has_glob) {
        if (FileExists(path)) results.emplace_back(path);
        return results;
    }

    std::string raw_root, item_path, err;
    if (!b->ParseUrl(path, raw_root, item_path, err)) return results;

    // Resolve root (with cache)
    std::string root_id;
    if (!cache_.GetRootId(b->Scheme(), raw_root, root_id)) {
        std::string tok;
        if (!GetToken(b->Scheme(), tok, err)) return results;
        if (!b->ResolveRoot(raw_root, tok, root_id, err)) return results;
        cache_.PutRootId(b->Scheme(), raw_root, root_id);
    }
    ...
    std::string static_prefix = (slash_before == std::string::npos)
                                  ? "/" : item_path.substr(0, slash_before);
    std::string glob_part     = (slash_before == std::string::npos)
                                  ? item_path : item_path.substr(slash_before + 1);
    bool recursive = glob_part.find("**") != std::string::npos;

    // Resolve starting folder
    CloudItem start_folder;
    if (!b->Stat(root_id, static_prefix, tok, start_folder, err)) return results;
    ...
    if (file_pat.empty()) file_pat = "*";

    GlobRecursive(*b, root_id, start_folder.id, scheme_url_prefix,
                   file_pat, recursive, results, tok);
