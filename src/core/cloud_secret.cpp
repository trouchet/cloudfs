...
void CloudSecretRegistry::Register(ExtensionLoader        &loader,
                                    CloudFileSystem        &cfs,
    ...
    try { loader.RegisterSecretType(st); } catch (...) {}
    ...
    loader.GetDatabaseInstance().GetSecretManager()
        .RegisterSecretFunction(std::move(f), OnCreateConflict::REPLACE_ON_CONFLICT);
}
