...
static CloudFileSystem* GetCFS(optional_ptr<TableFunctionInfo>) {
    return g_tf_cfs;
}
...
struct LsBindData : public FunctionData {
    ...
    LsBindData(CloudFileSystem* cfs_, std::string url_, bool rec, std::string pat)
...
static void WalkFolder(ICloudBackend& backend, const std::string& root_id,
                       const std::string& folder_id, const std::string& url_prefix, bool recursive,
                       const std::string& pattern, const std::string& token,
                       std::vector<std::pair<std::string, CloudItem>>& out) {
    std::string cursor, err;
    do {
        std::vector<CloudItem> batch;
        if (!backend.ListFolder(
                root_id, folder_id, token, [&](const CloudItem& c) { batch.push_back(c); }, cursor,
                err))
            break;
        for (auto& item : batch) {
            ...
            if (item.is_folder && recursive)
                WalkFolder(backend, root_id, item.id, item_url, recursive, pattern, token, out);
        }
    } while (!cursor.empty());
}
...
static unique_ptr<FunctionData> LsBind(ClientContext& ctx, TableFunctionBindInput& input,
    ...
    if (!target.is_folder) {
        ...
        while (!url_prefix.empty() && url_prefix.back() == '/')
            url_prefix.pop_back();
        WalkFolder(*b, root_id, target.id, url_prefix, recursive, pat, tok, data->rows);
    }
    return std::move(data);
}

static unique_ptr<GlobalTableFunctionState> LsInit(ClientContext&, TableFunctionInitInput&) {
...
struct DuBindData : public FunctionData {
    ...
    DuBindData(CloudFileSystem* cfs_, std::string url_) : cfs(cfs_), url(std::move(url_)) {}
    unique_ptr<FunctionData> Copy() const override {
...
static void WalkForDu(ICloudBackend& backend, const std::string& root_id,
                      const std::string& folder_id, const std::string& folder_url,
                      const std::string& token, std::vector<DuRow>& rows) {
    DuRow row;
...
static unique_ptr<GlobalTableFunctionState> DuInit(ClientContext&, TableFunctionInitInput&) {
