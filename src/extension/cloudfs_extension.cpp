...
static void LoadInternal(ExtensionLoader &loader) {
    ...
    CloudSecretRegistry::Register(loader, *g_cfs, "sharepoint", "oauth",
        ...
        [](ClientContext &, CreateSecretInput &in, CloudFileSystem &cfs) {
            ...
            auto auth = std::make_shared<SharePointDeviceCodeAuth>(
                ...
                get("client_id").empty() ? "04b07795-8542-4c45-a7a4-26c5a3e61e39" : get("client_id"),
                get("scope").empty()     ? "https://graph.microsoft.com/.default"  : get("scope"));
            cfs.SetAuth("spfs", auth);
        });

    CloudSecretRegistry::Register(loader, *g_cfs, "sharepoint", "token",
        {"tenant_id", "client_id", "scope", "token"},
        [](ClientContext &, CreateSecretInput &in, CloudFileSystem &cfs) {
            auto tok = in.options.count("token") ? in.options.at("token").ToString() : "";
            if (tok.empty()) throw InvalidInputException("sharepoint token: TOKEN required");
            cfs.SetAuth("spfs", std::make_shared<StaticTokenAuth>("sharepoint", tok));
        });

    // OneDrive — oauth + token
    CloudSecretRegistry::Register(loader, *g_cfs, "onedrive", "oauth",
        {"tenant_id", "client_id"},
        [](ClientContext &, CreateSecretInput &in, CloudFileSystem &cfs) {
            auto get = [&](const std::string &k) {
                auto it = in.options.find(k);
    ...
    CloudSecretRegistry::Register(loader, *g_cfs, "onedrive", "token",
        {"token"},
        [](ClientContext &, CreateSecretInput &in, CloudFileSystem &cfs) {
            auto tok = in.options.count("token") ? in.options.at("token").ToString() : "";
            if (tok.empty()) throw InvalidInputException("onedrive token: TOKEN required");
            cfs.SetAuth("odfs", std::make_shared<StaticTokenAuth>("onedrive", tok));
        });

    // Google Drive — oauth (user PKCE) + service_account (JWT)
    CloudSecretRegistry::Register(loader, *g_cfs, "gdrive", "oauth",
        ...
        [](ClientContext &, CreateSecretInput &in, CloudFileSystem &cfs) {
            ...
            cfs.SetAuth("gdfs", std::make_shared<GDriveOAuthProvider>(
                get("client_id"), get("client_secret")));
        });

    CloudSecretRegistry::Register(loader, *g_cfs, "gdrive", "service_account",
        {"key_json"},
        [](ClientContext &, CreateSecretInput &in, CloudFileSystem &cfs) {
            auto key = in.options.count("key_json") ? in.options.at("key_json").ToString() : "";
            if (key.empty()) throw InvalidInputException("gdrive service_account: KEY_JSON required");
            cfs.SetAuth("gdfs", std::make_shared<GDriveServiceAccountAuth>(key));
        });

    CloudSecretRegistry::Register(loader, *g_cfs, "gdrive", "token",
        {"token"},
        [](ClientContext &, CreateSecretInput &in, CloudFileSystem &cfs) {
            auto tok = in.options.count("token") ? in.options.at("token").ToString() : "";
            if (tok.empty()) throw InvalidInputException("gdrive token: TOKEN required");
            cfs.SetAuth("gdfs", std::make_shared<StaticTokenAuth>("gdrive", tok));
        });

    // Dropbox — oauth + token
    CloudSecretRegistry::Register(loader, *g_cfs, "dropbox", "oauth",
        {"app_key", "app_secret"},
        [](ClientContext &, CreateSecretInput &in, CloudFileSystem &cfs) {
            auto get = [&](const std::string &k) {
                auto it = in.options.find(k);
            ...
            cfs.SetAuth("dbxfs", std::make_shared<DropboxOAuthProvider>(
                get("app_key"), get("app_secret")));
        });

    CloudSecretRegistry::Register(loader, *g_cfs, "dropbox", "token",
        {"token"},
        [](ClientContext &, CreateSecretInput &in, CloudFileSystem &cfs) {
            auto tok = in.options.count("token") ? in.options.at("token").ToString() : "";
            if (tok.empty()) throw InvalidInputException("dropbox token: TOKEN required");
    ...
    //   "agent:"
    //   "password:mysecret"
    CloudSecretRegistry::Register(loader, *g_cfs, "sftp", "keyfile",
        {"host", "user", "key_path", "passphrase"},
        [](ClientContext &, CreateSecretInput &in, CloudFileSystem &cfs) {
            auto get = [&](const std::string &k) {
                auto it = in.options.find(k);
                return it == in.options.end() ? "" : it->second.ToString();
            ...
            if (!get("passphrase").empty()) tok += ":" + get("passphrase");
            if (tok == "keyfile:") throw InvalidInputException("sftp keyfile: KEY_PATH required");
            cfs.SetAuth("sftp", std::make_shared<SFTPAuth>(tok));
        });

    CloudSecretRegistry::Register(loader, *g_cfs, "sftp", "agent",
        {"host", "user"},
        [](ClientContext &, CreateSecretInput &, CloudFileSystem &cfs) {
            cfs.SetAuth("sftp", std::make_shared<SFTPAuth>("agent:"));
        });

    CloudSecretRegistry::Register(loader, *g_cfs, "sftp", "password",
        {"host", "user", "password"},
        [](ClientContext &, CreateSecretInput &in, CloudFileSystem &cfs) {
            auto it = in.options.find("password");
            std::string pw = it == in.options.end() ? "" : it->second.ToString();
            if (pw.empty()) throw InvalidInputException("sftp password: PASSWORD required");
            cfs.SetAuth("sftp", std::make_shared<SFTPAuth>("password:" + pw));
        });

    // ── VFS (cloudfs-agent) ───────────────────────────────────────────────────
    CloudSecretRegistry::Register(loader, *g_cfs, "vfs", "token",
        {"token"},
        [](ClientContext &, CreateSecretInput &in, CloudFileSystem &cfs) {
            auto it = in.options.find("token");
            std::string tok = it == in.options.end() ? "" : it->second.ToString();
            if (tok.empty()) throw InvalidInputException("vfs: TOKEN required");
    ...
    loader.RegisterFunction(ScalarFunction(
        ...
        [](DataChunk &, ExpressionState &, Vector &r) {
            r.SetValue(0, Value("cloudfs 0.1.0"));
        }));

    // providers() — no prefix, no collision
    {
        ScalarFunctionSet s("providers");
        s.AddFunction(ScalarFunction({}, LogicalType::VARCHAR,
            [](DataChunk &, ExpressionState &, Vector &r) {
                if (!g_cfs) { r.SetValue(0, Value("")); return; }
                std::string out;
    ...
    // clear_cache() and clear_cache(scheme) — no prefix, no collision
    {
        ...
        s.AddFunction(ScalarFunction({}, LogicalType::VARCHAR,
            [](DataChunk &, ExpressionState &, Vector &r) {
                if (g_cfs) g_cfs->GetCache().ClearAll();
        ...
        s.AddFunction(ScalarFunction({LogicalType::VARCHAR}, LogicalType::VARCHAR,
            [](DataChunk &args, ExpressionState &, Vector &r) {
                ...
                if (g_cfs) {
                    if (sc == "*" || sc == "all") g_cfs->GetCache().ClearAll();
                    else g_cfs->GetCache().ClearScheme(sc);
                }
                r.SetValue(0, Value("OK"));
            }));
        loader.RegisterFunction(s);
...
// ─────────────────────────────────────────────────────────────────────────────
void CloudFsExtension::Load(ExtensionLoader &loader) { LoadInternal(loader); }
std::string CloudFsExtension::Name()    { return "cloudfs"; }
