...
struct OAuth2TokenBundle {
    ...
    bool IsValid() const {
        return !access_token.empty() && std::chrono::system_clock::now() < expires_at;
    }
...
class OAuth2AuthBase : public ICloudAuthProvider {
  public:
    explicit OAuth2AuthBase(std::string provider_name) : provider_(std::move(provider_name)) {}
    ...
    void SerializeState(std::unordered_map<std::string, std::string>& out) const override {
        ...
        auto epoch =
            std::chrono::duration_cast<std::chrono::seconds>(token_.expires_at.time_since_epoch())
                .count();
  ...
  protected:
    ...
    static std::string PostForm(const std::string& url, const std::string& body, std::string& err);
