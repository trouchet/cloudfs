...
public:
    ...
    ICloudBackend  *BackendForPublic(const std::string &url) {
        return BackendForNoThrow(url);
    }
    // Resolve root_id for a (backend, raw_root) pair, caching the result
    bool GetRootIdPublic(ICloudBackend *b,
        ...
        if (!GetToken(b->Scheme(), out_token, err)) return false;
        if (!b->ResolveRoot(raw_root, out_token, out_root_id, err)) return false;
        cache_.PutRootId(b->Scheme(), raw_root, out_root_id);
        return true;
    }
    bool GetTokenPublic(const std::string &scheme,
                         std::string &out_token,
                         std::string &err) {
    ...
    bool ListFiles(const string &dir,
...
private:
    ...
    bool ResolvePath(ICloudBackend &backend, const std::string &url,
                     CloudItem &out_item, std::string &out_root_id, std::string &err);

    // Ensure download URL is fresh for a CloudItem
    bool EnsureDownloadUrl(ICloudBackend &backend,
    ...
    void GlobRecursive(ICloudBackend &backend,
                        const std::string &root_id,
...
public:
    CloudFileHandle(CloudFileSystem &fs, const std::string &path,
                    ICloudBackend &backend,
                    const CloudItem &item, const std::string &root_id,
                    FileOpenFlags flags);
