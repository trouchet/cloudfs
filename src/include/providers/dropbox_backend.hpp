...
class DropboxOAuthProvider : public OAuth2AuthBase {
  public:
    DropboxOAuthProvider(std::string app_key, std::string app_secret)
        : OAuth2AuthBase("dropbox"), app_key_(std::move(app_key)),
  ...
  private:
    ...
    static constexpr const char* kTokenUrl = "https://api.dropboxapi.com/oauth2/token";
    static constexpr const char* kDeviceUrl = "https://api.dropboxapi.com/oauth2/authorize";
};
