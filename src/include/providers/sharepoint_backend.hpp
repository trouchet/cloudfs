...
private:
    ...
    struct SiteInfo { std::string site_id; std::string drive_id; };
...
public:
    SharePointDeviceCodeAuth(std::string tenant_id,
                              std::string client_id,
                              std::string scope)
        : OAuth2AuthBase("sharepoint"),
...
private:
    bool PollDeviceCode(const std::string &device_code,
                         int interval, int expires_in, std::string &err);
