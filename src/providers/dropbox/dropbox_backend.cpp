...
bool DropboxBackend::ParseUrl(const std::string &url,
                               std::string &out_root, std::string &out_path,
                               std::string &err) const {
    const std::string pfx = "dbxfs://";
    if (url.substr(0, pfx.size()) != pfx) { err = "not dbxfs://"; return false; }
...
bool DropboxBackend::ResolveRoot(const std::string &root, const std::string &,
                                  std::string &out_id, std::string &err) {
    out_id = root; return true; // Dropbox uses paths, not IDs for root
}
...
bool DropboxBackend::Stat(const std::string &, const std::string &path,
    ...
    if (resp.status == 409) { err = "not found: " + path; return false; }
    if (!resp.ok()) { err = "stat failed " + std::to_string(resp.status); return false; }
    out = ParseMetadata(resp.body);
    return true;
}

int64_t DropboxBackend::ReadRange(const CloudItem &item, const std::string &,
...
bool DropboxBackend::RefreshDownloadUrl(const CloudItem &item, const std::string &,
                                         const std::string &tok,
                                         std::string &out_url, std::string &err) {
    HttpRequest req;
    req.method = "POST"; req.url = std::string(kApiBase) + "/files/get_temporary_link";
    req.SetBearerAuth(tok); req.headers["Content-Type"] = "application/json";
    req.body = "{\"path\":\"" + item.path + "\"}";
    auto resp = http_.Execute(req);
    if (!resp.ok()) { err = "get_temporary_link failed"; return false; }
...
bool DropboxBackend::ListFolder(const std::string &, const std::string &folder_id,
    ...
    auto resp = http_.Execute(req);
    if (!resp.ok()) { err = "list_folder failed " + std::to_string(resp.status); return false; }

    for (auto &j : JsonUtil::GetArray(resp.body, "entries")) cb(ParseMetadata(j));
...
bool DropboxBackend::ListFolderRecursive(const std::string &root, const std::string &folder_id,
    ...
    HttpRequest req;
    req.method = "POST"; req.SetBearerAuth(tok); req.headers["Content-Type"] = "application/json";
    req.url    = std::string(kApiBase) + "/files/list_folder";
    req.body   = "{\"path\":\"" + (folder_id == "root" || folder_id.empty() ? "" : folder_id) +
    ...
    do {
        auto resp = http_.Execute(req);
        if (!resp.ok()) { err = "list_folder recursive failed"; return false; }
        for (auto &j : JsonUtil::GetArray(resp.body, "entries")) cb(ParseMetadata(j));
        bool has_more = JsonUtil::GetBool(resp.body, "has_more");
        if (!has_more) break;
        req.url  = std::string(kApiBase) + "/files/list_folder/continue";
...
bool DropboxBackend::CreateUploadSession(const std::string &, const std::string &,
                                          const std::string &, int64_t,
                                          const std::string &tok,
                                          CloudUploadSession &out, std::string &err) {
    HttpRequest req;
    req.method = "POST"; req.SetBearerAuth(tok);
    req.url    = std::string(kContentBase) + "/files/upload_session/start";
    req.headers["Dropbox-API-Arg"] = "{\"close\":false}";
    req.headers["Content-Type"]    = "application/octet-stream";
    req.body = "";
    auto resp = http_.Execute(req);
    if (!resp.ok()) { err = "upload_session/start failed"; return false; }
...
bool DropboxBackend::UploadChunk(const CloudUploadSession &s, const char *data,
                                  int64_t off, int64_t sz, bool last,
                                  const std::string &tok, std::string &err) {
    HttpRequest req;
    req.method  = "POST"; req.SetBearerAuth(tok);
    req.headers["Content-Type"] = "application/octet-stream";
    req.body_data = data; req.body_size = sz;
    ...
    if (!last) {
        ...
        std::string arg = "{\"cursor\":{\"session_id\":\"" + s.upload_url +
                          ...
                          "\"commit\":{\"path\":\"" + (s.item_id.empty() ? "/upload" : s.item_id) +
                          "\",\"mode\":\"overwrite\"}}";
        req.headers["Dropbox-API-Arg"] = arg;
    }
...
bool DropboxBackend::DeleteItem(const std::string &, const std::string &id,
                                 const std::string &tok, std::string &err) {
    HttpRequest req;
    req.method = "POST"; req.SetBearerAuth(tok); req.headers["Content-Type"] = "application/json";
    ...
    req.body   = "{\"path\":\"" + id + "\"}";
    auto resp  = http_.Execute(req);
    if (!resp.ok()) { err = "delete failed " + std::to_string(resp.status); return false; }
    return true;
}
...
bool DropboxBackend::CreateFolder(const std::string &, const std::string &parent_id,
    ...
    auto resp  = http_.Execute(req);
    if (!resp.ok()) { err = "create_folder failed"; return false; }
    out.id = parent_id + "/" + name; out.name = name; out.is_folder = true;
    return true;
}
...
bool DropboxBackend::CopyItem(const std::string &, const std::string &src_id,
    ...
    req.body   = "{\"from_path\":\"" + src_id + "\",\"to_path\":\"" +
