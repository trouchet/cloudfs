...
bool GDriveOAuthProvider::AcquireToken(std::string &err) {
    ...
    if (device_code.empty()) { err = "no device_code from Google"; return false; }
    ...
    while (std::chrono::system_clock::now() < deadline) {
        ...
        std::string b = "client_id=" + client_id_ +
        ...
        if (ec == "authorization_pending" || ec == "slow_down") { if(ec=="slow_down")interval+=5; continue; }
        if (!ec.empty()) { err = ec; return false; }
        std::string at = JsonGet(r, "access_token");
        if (at.empty()) continue;
        token_.SetFromResponse(at, JsonGet(r,"refresh_token"), std::max(JsonGetInt(r,"expires_in"),3600));
        std::cerr << "[cloudfs Google Drive] Authenticated.\n";
        return true;
    }
    err = "timed out"; return false;
}
...
bool GDriveOAuthProvider::RefreshToken(std::string &err) {
    ...
    if (resp.empty() || !JsonGet(resp,"error").empty()) return false;
...
GDriveServiceAccountAuth::GDriveServiceAccountAuth(std::string key_json) {
    ...
    auto get = [&](const std::string &k) -> std::string {
        ...
        if (pos >= key_json.size() || key_json[pos] != '"') return "";
        ++pos; std::string v;
        while (pos < key_json.size() && key_json[pos] != '"') {
            if (key_json[pos]=='\\' && pos+1<key_json.size()) { ++pos; v+=key_json[pos]; }
            else v+=key_json[pos]; ++pos;
        }
        return v;
    };
...
bool GDriveServiceAccountAuth::GetAccessToken(std::string &out, std::string &err) {
    ...
    if (token_.IsValid() && !token_.IsExpiringSoon()) { out = token_.access_token; return true; }
    if (!RefreshServiceAccountToken(err)) return false;
    out = token_.access_token; return true;
}
