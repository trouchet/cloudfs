...
bool GDriveBackend::ParseUrl(const std::string& url, std::string& out_root, std::string& out_path,
...
bool GDriveBackend::ResolveRoot(const std::string& root, const std::string& tok,
    ...
    if (root == "MyDrive" || root == "me" || root.empty()) {
        out_id = "root";
        return true; // "root" is the magic alias for user's root folder
    }
...
bool GDriveBackend::ResolvePathWalk(const std::string& start_folder_id,
    ...
    for (size_t i = 0; i < segments.size(); ++i) {
        ...
        std::string q_url = kApiBase + "/files?q='" + current_id + "'+in+parents+and+name='" + seg +
                            "'+and+trashed=false"
                            "&fields=files(id,name,size,mimeType,md5Checksum)"
                            "&supportsAllDrives=true&includeItemsFromAllDrives=true";
        auto resp = http_.Get(q_url, tok);
        if (!resp.ok()) {
            err = "walk failed at '" + seg + "': " + std::to_string(resp.status);
            return false;
        }
...
bool GDriveBackend::Stat(const std::string& root, const std::string& path, const std::string& tok,
    ...
    if (!ResolvePathWalk(root, segs, tok, out, err))
        return false;
    out.path = path;
    return true;
}

int64_t GDriveBackend::ReadRange(const CloudItem& item, const std::string&, const std::string& tok,
...
bool GDriveBackend::ListFolder(const std::string& root, const std::string& folder_id,
                               ...
                               std::string& err) {
    std::string url =
        cursor.empty() ? kApiBase + "/files?q='" + folder_id +
...
bool GDriveBackend::CreateUploadSession(const std::string& root, const std::string& parent_id,
    ...
    if (resp.status != 200) {
        err = "initiate upload failed " + std::to_string(resp.status);
        return false;
    }
...
bool GDriveBackend::UploadChunk(const CloudUploadSession& s, const char* data, int64_t off,
                                int64_t sz, bool last, const std::string&, std::string& err) {
    std::unordered_map<std::string, std::string> hdrs;
    int64_t total = last && s.total_size_bytes > 0 ? s.total_size_bytes : -1;
    char cr[96];
    snprintf(cr, sizeof(cr), "bytes %lld-%lld/%s", (long long)off, (long long)(off + sz - 1),
...
bool GDriveBackend::DeleteItem(const std::string&, const std::string& id, const std::string& tok,
                               std::string& err) {
