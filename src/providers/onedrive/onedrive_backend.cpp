...
bool OneDriveBackend::ParseUrl(const std::string& url, std::string& out_root, std::string& out_path,
...
bool OneDriveBackend::ResolveRoot(const std::string& root, const std::string& tok,
    ...
    if (!resp.ok()) {
        err = "get drive failed (" + std::to_string(resp.status) + ")";
        return false;
    }
...
bool OneDriveBackend::Stat(const std::string& root, const std::string& path, const std::string& tok,
...
bool OneDriveBackend::CreateUploadSession(const std::string& root, const std::string& parent_id,
                                          const std::string& name, int64_t, const std::string& tok,
    ...
    auto resp =
        http_.Post(url, tok, "{\"item\":{\"@microsoft.graph.conflictBehavior\":\"replace\"}}");
    if (!resp.ok()) {
...
bool OneDriveBackend::UploadChunk(const CloudUploadSession& s, const char* data, int64_t off,
                                  int64_t sz, bool, const std::string&, std::string& err) {
