...
SFTPBackend::~SFTPBackend() {
    for (auto& [k, c] : connections_)
        c.Close();
}

// ─── URL parsing ──────────────────────────────────────────────────────────────
// sftp://user@host:port/path
bool SFTPBackend::ParseUrl(const std::string& url, std::string& out_root, std::string& out_path,
...
static bool ParseRoot(const std::string& root, std::string& out_user, std::string& out_host,
                      int& out_port, std::string& err) {
    out_port = 22;
    auto at = root.rfind('@');
    if (at == std::string::npos) {
        err = "sftp root must be user@host[:port]";
        return false;
    }
...
SSHConnection& SFTPBackend::GetConnection(const std::string& root, const std::string& token,
                                          std::string& err) {
    ...
    if (libssh2_session_handshake(conn.session, conn.socket_fd) != 0) {
        char* msg;
        libssh2_session_last_error(conn.session, &msg, nullptr, 0);
    ...
    if (token.substr(0, 8) == "keyfile:") {
        ...
        rc = libssh2_userauth_publickey_fromfile(conn.session, user.c_str(), pubkey.c_str(),
                                                 keyfile.c_str(),
...
// ─── Stat ─────────────────────────────────────────────────────────────────────
bool SFTPBackend::Stat(const std::string& root, const std::string& path, const std::string& token,
...
int64_t SFTPBackend::ReadRange(const CloudItem& item, const std::string& root,
                               const std::string& token, int64_t off, int64_t len, char* buf,
                               std::string& err) {
    auto& conn = GetConnection(root, token, err);
    if (!conn.IsValid())
        return -1;

    LIBSSH2_SFTP_HANDLE* fh = libssh2_sftp_open(conn.sftp, item.path.c_str(), LIBSSH2_FXF_READ, 0);
    if (!fh) {
        err = "sftp open failed: " + item.path;
        return -1;
    }
...
bool SFTPBackend::ListFolder(const std::string& root, const std::string& folder_id,
    ...
    if (!dh) {
        err = "sftp opendir failed: " + folder_id;
        return false;
    }
    ...
    while (true) {
        int rc = libssh2_sftp_readdir_ex(dh, name_buf, sizeof(name_buf), longentry,
...
bool SFTPBackend::UploadChunk(const CloudUploadSession& s, const char* data, int64_t off,
    ...
    LIBSSH2_SFTP_HANDLE* fh = libssh2_sftp_open(conn.sftp, s.upload_url.c_str(), open_flags, 0644);
    if (!fh) {
        err = "sftp open for write failed: " + s.upload_url;
        return false;
    }
...
bool SFTPBackend::DeleteItem(const std::string& root, const std::string& id,
    ...
    if (libssh2_sftp_unlink(conn.sftp, id.c_str()) == 0)
        return true;
    if (libssh2_sftp_rmdir(conn.sftp, id.c_str()) == 0)
        return true;
