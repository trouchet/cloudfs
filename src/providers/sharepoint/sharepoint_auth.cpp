...
bool SharePointDeviceCodeAuth::AcquireToken(std::string& err) {
    ...
    if (device_code.empty()) {
        err = "no device_code in response: " + resp.substr(0, 200);
        return false;
    }
...
bool SharePointDeviceCodeAuth::PollDeviceCode(const std::string& device_code, int interval,
                                              int expires_in, std::string& err) {
    ...
    while (std::chrono::system_clock::now() < deadline) {
        ...
        std::string body = "client_id=" + client_id_ + "&device_code=" + device_code +
        ...
        if (!ec.empty()) {
            err = "auth error: " + ec + " — " + JsonGet(resp, "error_description");
            return false;
        }

        std::string at = JsonGet(resp, "access_token");
        if (at.empty()) {
            err = "no access_token: " + resp.substr(0, 200);
            return false;
        }
        token_.SetFromResponse(at, JsonGet(resp, "refresh_token"),
...
bool SharePointDeviceCodeAuth::RefreshToken(std::string& err) {
    ...
    if (!JsonGet(resp, "error").empty()) {
        err = "refresh: " + JsonGet(resp, "error");
        return false;
    }
    std::string at = JsonGet(resp, "access_token");
    if (at.empty())
