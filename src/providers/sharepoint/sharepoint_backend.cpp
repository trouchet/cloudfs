...
bool SharePointBackend::ParseUrl(const std::string &url,
                                  std::string &out_root, std::string &out_path,
                                  std::string &err) const {
    const std::string pfx = "spfs://";
    if (url.substr(0, pfx.size()) != pfx) { err = "not a spfs:// URL"; return false; }
    std::string rest = url.substr(pfx.size());
    auto slash1 = rest.find('/');
    if (slash1 == std::string::npos) { out_root = "https://" + rest; out_path = "/"; return true; }
...
bool SharePointBackend::ResolveRoot(const std::string &root, const std::string &tok,
    ...
    auto pipe = root.find('|');
    if (pipe == std::string::npos) { err = "invalid spfs root: " + root; return false; }
    ...
    if (!resp.ok()) { err = "get site failed (" + std::to_string(resp.status) + "): " + resp.body.substr(0, 200); return false; }
    std::string site_id = JsonUtil::GetString(resp.body, "id");

    auto drive_resp = http_.Get("https://graph.microsoft.com/v1.0/sites/" + site_id + "/drive", tok);
    if (!drive_resp.ok()) { err = "get drive failed"; return false; }
...
// ─── Upload session ───────────────────────────────────────────────────────────
bool SharePointBackend::CreateUploadSession(const std::string &root,
...
bool SharePointBackend::UploadChunk(const CloudUploadSession &s, const char *data,
...
bool SharePointBackend::CreateFolder(const std::string &root, const std::string &parent_id,
    ...
    std::string body = "{\"name\":\"" + name + "\",\"folder\":{},\"@microsoft.graph.conflictBehavior\":\"replace\"}";
    auto resp = http_.Post(url, tok, body);
    if (!resp.ok()) { err = "createFolder failed"; return false; }
    out.id = JsonUtil::GetString(resp.body, "id");
    out.name = name; out.is_folder = true;
    return true;
}
...
bool SharePointBackend::CopyItem(const std::string &root, const std::string &src_id,
                                  ...
                                  const std::string &tok, std::string &err) {
    std::string url  = "https://graph.microsoft.com/v1.0/drives/" + root + "/items/" + src_id + "/copy";
    std::string body = "{\"parentReference\":{\"id\":\"" + dst_parent_id + "\"},\"name\":\"" + dst_name + "\"}";
    auto resp = http_.Post(url, tok, body);
    if (resp.status != 202) { err = "copy failed (" + std::to_string(resp.status) + ")"; return false; }
    return true;
}
