...
// ─── URL parsing ──────────────────────────────────────────────────────────────
bool VFSBackend::ParseUrl(const std::string& url, std::string& out_root, std::string& out_path,
...
// ─── Stat ─────────────────────────────────────────────────────────────────────
bool VFSBackend::Stat(const std::string& root, const std::string& path, const std::string& token,
                      CloudItem& out, std::string& err) {
    std::string url = root + "/v1/stat?" + UrlUtil::BuildQuery({{"path", path}});
...
int64_t VFSBackend::ReadRange(const CloudItem& item, const std::string& root,
                              ...
                              std::string& err) {
    std::string url = root + "/v1/read?" + UrlUtil::BuildQuery({{"path", item.path}});
    return http_.ReadRange(url, token, off, len, buf, err);
}
...
bool VFSBackend::ListFolder(const std::string& root, const std::string& folder_id,
                            ...
                            std::string& err) {
    std::vector<std::pair<std::string, std::string>> params = {{"path", folder_id}};
    if (!cursor.empty())
        params.push_back({"cursor", cursor});
    std::string url = root + "/v1/list?" + UrlUtil::BuildQuery(params);
...
// ─── Upload session ───────────────────────────────────────────────────────────
bool VFSBackend::CreateUploadSession(const std::string& root, const std::string& parent_id,
...
bool VFSBackend::UploadChunk(const CloudUploadSession& s, const char* data, int64_t off,
    ...
    if (resp.status == 202 || resp.status == 201 || resp.status == 200)
        return true;
    err = "upload chunk failed " + std::to_string(resp.status);
    return false;
}
...
bool VFSBackend::DeleteItem(const std::string& root, const std::string& id,
    ...
    std::string url = root + "/v1/delete?" + UrlUtil::BuildQuery({{"path", id}});
...
bool VFSBackend::CreateFolder(const std::string& root, const std::string& parent_id,
                              ...
                              std::string& err) {
    std::string path = parent_id + "/" + name;
    std::string body = JsonUtil::MakeObject({{"path", path}});
    auto resp = http_.Post(root + "/v1/mkdir", token, body);
    if (!resp.ok()) {
        err = "mkdir failed";
        return false;
    }
